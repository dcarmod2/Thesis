...
\subsection{Hermitian Forms on Semilocal Rings}\label{sec:herm_form_semiloc}
...
this chapter is to define a sheaf on $\Sm{S}^{C_2}$, denoted $\RGr_V$, which represents
...
As in \cite{SchTri}, we first compare $\RGr_d(\mbb H^\infty)$ to the isovariant \'etale
...
isovariant \'etale topology, hermitian vectors bundles are isometric
to hyperbolic space. This will utilize some of the analysis of
hermitian forms over semi-local rings from section
\ref{sec:herm_form_semiloc}.

We then compare $B_{isoEt}O$ (the colimit of the $B_{isoEt}O(d)$) to the Grothendieck-Witt space defined in
chapter \ref{chap:higher_GW} by viewing them both as group completions
and comparing their homology. 



\todo{Why does one want this?}
...
\[
\xymatrix{\RGr_d(V) \ar[r]\ar[d] & \RGr(V) \ar[d]^{\tr} \\ S
  \ar[r]^d & \mbb A^1_S}
\]

Here the trace map is induced under the equivalence of categories
between locally free sheaves and vector bundles by the map 
\[
 \RGr(V) \hookrightarrow \uline{\Hom}_{\mc O_S}(V,V) \xrightarrow{\sim} V^* \otimes V
 \xrightarrow{ev} \mathcal O_S.
\]
The
...
induces an action on $\RGr_d(V)$. \todo{ask jeremiah about this}
\end{remark}
...
\]
which is a local equivalence of isovariant \'etale sheaves, and hence
...
\begin{proof}
This follows from the previous proposition, Corollary
\ref{cor:RGr_S_eq}, and Corollary \ref{cor:htpy_RGr_compare}.
\end{proof}
...
the non-equivariant case. \todo{Is there an actual description of this
element?} 
