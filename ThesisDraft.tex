...
I'd like to first thank my advisor Jeremiah Heller, as without his expertise
and support this thesis never would have been written. The homotopy
theory community in general was always welcoming, but I'd like
especially to thank: Cary Malkiewich, who made significant efforts to
engage with and mentor graduate students when he was a postdoc at
UIUC; my academic brothers Brian and Tsutomu; William Balderrama for
many productive conversations;  and Elden Elmanto, who helped me
significantly when I was bridging the gap between coursework and
research. 

I'd also like to thank my partner Hanna for being thoughtful and
understanding, and my friend Chris who was always willing to teach me
about interesting results in various areas of mathematics. 

Finally, thanks to Richard Sowers, Yuliy Baryshnikov, and Fatima
Husain, who believed in my abilities enough to let me try weird
topological things with data sets. 
...
which is Noetherian of finite Krull dimension, has an ample
family of line bundles, and has $\frac{1}{2} \in \Gamma(S,\mc O_S)$. First we review the definition of the equivariant
...
The content in this chapter is a selection of relevant content from
...
\cite{SchTri} to show that the Grothendieck-Witt space $\Omega^\infty GW$ is representable by a
...
\begin{theorem}
Let $S$ be a regular Noetherian scheme of finite Krull dimension with an ample
family of line bundles and $\frac{1}{2} \in S$. Let $\Omega^\infty GW$ denote the Grothendieck-Witt space on $\Sm{S,qp}^{C_2}$, and let
...
\xrightarrow{\sim}  L_{\mathrm{mot}}\Omega^\infty GW.
\]
Here $L_{\mathrm{mot}}$ denotes motivic localization.
\end{theorem}
...
\begin{theorem}
Let $S$ be a Noetherian scheme of finite Krull dimension with an ample
family of line bundles and $\frac{1}{2} \in S$. There are
equivalences of motivic spaces on $\Sm{S,qp}^{C_2}$
\[
...
\begin{theorem}
Let $S$ be a Noetherian scheme of finite Krull dimension with an ample
family of line bundles and $\frac{1}{2} \in S$. Then $L_{\mbb A^1}\mbb GW$ lifts to an $E_\infty$ motivic spectrum, denoted
$\KR$, over $\Sm{S,qp}^{C_2}$.
\end{theorem}
...
\begin{theorem}
Let $S$ be a Noetherian scheme of finite Krull dimension with an ample
family of line bundles and $\frac{1}{2} \in S$. Then the
homotopy Hermitian $K$-theory spectrum of rings with involution $L_{\mbb A^1}\mbb
GW$ satisfies descent for the equivariant cdh topology on $\Sm{S,qp}^{C_2}$.
\end{theorem}
...
$\HH(S)$ over a Noetherian base scheme $S$ with finite Krull
dimension, with an ample family of line bundles, and with $\frac{1}{2}
\in \Gamma(S,\mc O_S)$. Throughout we'll work with the category of
smooth, quasi-projective $C_2$-schemes over $S$,
denoted $\Sm{S,qp}^{C_2}$.
...
\begin{notation}
Let $S$ be a  $G$-scheme. The equivariant \'etale topology on $\Sm{S,qp}^{G}$
...
\begin{definition}
The equivariant Nisnevich $cd$-structure on $\Sm{S,qp}^G$ is the
collection of distinguished equivariant Nisnevich squares in $\Sm{S,qp}^G$.
\end{definition}
...
An abstract blow-up square is a cartesian square in $\Sm{S,qp}^G$
\[
...
an isomorphism $(\widetilde X - \widetilde Z) \cong (X-Z)$.
\end{definition}
...
\begin{definition}
A presheaf $F$ on $\Sm{S,qp}^G$ is called \emph{homotopy invariant} if
the projection $\mbb A^1_S \rightarrow S$ induces an equivalence $F(X)
\simeq F(X \times \mbb A^1)$. Denote by $\mc
P_{\mathrm{htp}}(\Sm{S,qp}^{G}) \subset \mc P(\Sm{S,qp}^G)$ the full
subcategory spanned by the homotopy invariant presheaves. Denote by
$L_{\mathrm{htp}}$ the corresponding localization endofunctor of $\mc P(\Sm{S,qp}^G)$.
\end{definition}
...
\begin{definition}
A presheaf $F$ on $\Sm{S,qp}^G$ is called \emph{Nisnevich excisive} if:
\begin{itemize}
\item $F(\emptyset)$ is contractible;
\item for every equivariant Nisnevich square $Q$ in $\Sm{S,qp}^G$, $F(Q)$ is
  cartesian. 
\end{itemize}

Denote by $\mc P_{\mathrm{Nis}}(\Sm{S,qp}^G) \subset \mc P(\Sm{S,qp}^G)$ the
...
presheaf on $\Sm{S,qp}^G$ that is homotopy invariant and Nisnevich
excisive. Denote by $\HH^G(S) \subset \mc P(\Sm{S,qp}^G)$ the full
...
\begin{definition}
Let $X$ be a Noetherian scheme of finite Krull dimension with an ample
family of line bundles, and let
...
If $Z$ is an invariant closed subset of $X$, then the duality on
$\sPerf(X)$ restricts to a duality on the subcategory of complexes
supported on $Z$, $\sPerf_Z(X)$. We define 
\[
GW^{[i]}(X \text{ on } Z) = GW (\sPerf_Z(X),\text{q. iso}, \ast^i,\can).
\]
\end{definition}
...
The goal of
this chapter is to define a sheaf on $\Sm{S,qp}^{C_2}$, denoted $\RGr$, which represents
...
category of $C_2$-schemes smooth over $S$, $\Sm{S,qp}^{C_2}$. To
...
representing Hermitian $K$-theory on $\Sm{S,qp}^{C_2}$ will be a colimit of schemes
...
Let $\mc F$ be a presheaf on $\Sm{S,qp}$ and let $a : \mc F \implies
\mc F$ be a natural transformation such that $a \circ a = id_{\mc F}$. Then
there's an associated presheaf on $\Sm{S,qp}^{C_2}$ defined by the formula
...
We'll define a presheaf $\RGr : (\Sm{S,qp}^{C_2})^{op} \rightarrow \Set$
by first defining a presheaf on $\Sm{S,qp}$, showing that it's
representable, equipping with an action, then taking the corresponding
representable functor on $\Sm{S,qp}^{C_2}$. We can then extend to an
...
follows: 
 ...
 Fix an object $X \in \Sm{S,qp}$. Define
\[
...
\Sm{S,qp}^{C_2} \rightarrow \Set$. To determine its values on a
...
\[
\sigma^*\Hom(M,\mc O_x) \cong
\Hom(M,\mc O_X)
\] and $\sigma^*f^*V \cong f^*V$,
...
in $V'$. Let $\mc H : P \rightarrow \mathrm{Fun}(\Sm{S,qp}^{C_2,
...
% first define the pseudo-presheaf $\mc F_{(V,\phi)} : \Sm{S,qp} \rightarrow \Set$ by 
% \[
...
% \begin{definition}
% Fix $(V,\phi)$ as above. Let $\mc A$ be a small category equivalent to $(\Sm{S,qp})_{/X}$. A big
...
% $\BigVB{-}$ on $\Sm{S,qp}$. By applying $\pi_0$, we obtain a presheaf of sets.
% \end{lemma}
...
% on $\Sm{S,qp}^{C_2}$. We claim that this presheaf represents
% non-degenerate Hermitian sub-bundles of $(V,\phi)$.

% \begin{lemma}
% Given $X \in \Sm{S,qp}^{C_2}$
% \[
...
$\Sm{S,qp}^{C_2}$. For integer $d \geq 0$, define
\[
...
Let $\mc F : \Sm{S,qp}^{C_2} \rightarrow Gpd$ be the sheaf which assigns
...
of Lemma \ref{lem:RGr_GW} shows that 
\[
L_{\mbb A^1}GW \cong
L_{\mbb A^1} \colim_n B_{isoEt}O(\mbb H^n)
\]
 over Noetherian base rings of finite Krull dimension. In particular,
there's a motivic equivalence \[
\colim_n B_{isoEt}O(\mbb H^n) \cong GW
\]
 over Noetherian
...
Let $S$ be a Noetherian scheme of finite Krull dimension with
$\frac{1}{2} \in S$. There are
equivalences of motivic spaces on $\Sm{S,qp}^{C_2}$
\[
...
Let $X$ be a scheme with trivial involution with an ample family of line
...
Considering $GW$ as a presheaf of spectra on $\Sm{S,qp}^{C_2}$ it follows from
...
GW^{[n+1]}
\]
as presheaves of spectra on $\Sm{S,qp}^{C_2}$. In particular, by the
...
\begin{lemma}
$\mbb GW$ is Nisnevich excisive on the category of smooth schemes with
an ample family of line bundles over $S$. 
\end{lemma}

\begin{proof}
Recall that the distinguished squares defining the equivariant
Nisnevich cd-structure are cartesian squares in $\Sm{S,qp}^G$ 
...
\]
Because $\mbb GW$ is invariant under derived
equivalences \cite[Theorem 8.9]{Schder}, it follows that $p^*$ induces
an isomorphism on Grothendieck-Witt groups. Noting that $U$ and the closed
subset $Z = X-A$ are $G$-invariant, the localization
sequence \cite[Theorem
9.5]{Schder} generalizes to our setting and identifies $\mbb GW(\mathrm{sPerf}_Z(X))$ and $\mbb
GW(\mathrm{sPerf}_Z(Y))$ as the horizontal homotopy fibers. This allows us to conclude the result. 
\end{proof}
...
equivalence of (pre)sheaves of spectra on $\Sm{S,qp}^{C_2}$. 
\end{lemma}
...
$\Sm{S,qp}^{C_2}$ via the cup product defined in \cite[Remark 5.1]{Schder}. By
...
\rightarrow L_{\mbb A^1} GW$, viewed as an element of $\Sp(\mc P_{\mbb
  A^1}(\Sm{S,qp}^{C_2}))_{/L_{\mbb A^1}GW}$ is 3-symmetric.
\end{lemma}
...
Replacing $GW$ with its connective cover $GW_{\geq 0}$, the same
reasoning yields:
\begin{porism}\label{por:GW_per}
The canonical map $GW_{\geq 0} \rightarrow
Q_{\beta}L_{\mathrm{mot}}GW_{\geq 0}$ is the universal
map to a homotopy invariant, Nisnevich excisive, and Bott periodic
$GW_{\geq 0}$-module. In particular
\[
L_{\mbb A^1}\mbb GW = Q_{\beta}L_{\mathrm{mot}}GW_{\geq 0}
\]
\end{porism}
\begin{proof}
\end{proof}
...
Fix $X \in \Sm{S,qp}$, and let $\KGL_X$ denote the restriction of $\KGL$
...
Consider the stable $\infty$-category valued functor $\Sm{S,qp}
...
Hermitian $K$-theory $L_{\mbb A^1}\mbb GW$ on $\Sm{S,qp}^{C_2}$ for
$S$ a Noetherian scheme of finite Krull dimension with an ample
family of line bundles and $\frac{1}{2} \in S$.

Let $\HH(S)$ denote the motivic homotopy $\infty$-category on
$\Sm{S,qp}^{C_2}$. By Corollary \ref{cor:MotLoc_GW}, we can write $\KR$ as the image under the localization functor
\[
QL_{\mathrm{mot}} : \mathrm{Stab}^{\mathrm{lax}}_{T^\rho} \Sp(\mc
P(\Sm{S,qp}^{C_2})) \rightarrow \mathrm{Stab}_{T^\rho} \Sp(\underline
H) \simeq \SH^{C_2}
\]
...
\begin{definition}
For $X \in \Sm{S,qp}^{C_2}$, let $\KR_X$ denote the restriction of the
motivic spectrum $\KR$ to $\Sm{X,qp}^{C_2}$.
\end{definition}

We want to show that $\KR$ is a cdh sheaf on $\Sm{S,qp}^{C_2}$. By first
...
each $f : D \rightarrow X \in \Sm{S,qp}^{C_2}$, the restriction map
\[
...
hyperbolic rings). Since $L_{\mathrm{Zar}}$ preserves finite products,
by \cite[Lemma 5.5]{cdhdesc}, the map remains a Zariski
...
Fix a map $f : D \rightarrow X$ in $\Sm{S,qp}^{C_2}$. Again by 
\cite[Lemma 5.5]{cdhdesc}, since the pullback $f^* : \mc P(\Sm{X}^{C_2})
...
$E_\infty$-monoids in $\mc P(\Sm{D}^{C_2})$. Moving to the category of
connective spectra, It follows that pullback agrees with restriction for $GW_{\geq
  0}$. Because the localization
functor $QL_{\mathrm{mot}}$ is also compatible with the base change
$f^*$, it follows that each arrow 
\[
f^*(QL_{\mathrm{mot}}GW_{\geq 0}|\Sm{X}^{C_2}) \rightarrow
QL_{\mathrm{mot}}(f^*GW_{\geq 0}|\Sm{X}^{C_2}) \rightarrow
QL_{\mathrm{mot}}(GW_{\geq 0}|\Sm{D}^{C_2})
\]
is a motivic equivalence. Finally, Porism \ref{por:GW_per} tells us
that $QL_{\mathrm{mot}}GW_{\geq 0} \simeq QL_{\mathrm{mot}}GW$,  so we've proved
...
% on $\Sm{S,qp}^{C_2}$ given by $f : X \rightarrow S \mapsto BigVB(S)$, or some
