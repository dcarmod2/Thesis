...
Let 
\[
\widetilde{GW} = \fib(GW(\mbb P^\sigma) \rightarrow GW([1:1])),
\]
be the fiber of the map induced by the equivariant inclusion $[1:1]
\hookrightarrow \mbb P^\sigma$,
and define $\widetilde{\mathbb GW}$ similarly. Then it follows from
theorem \ref{thm:proj_inv} that $\widetilde{GW}(\mbb P^\sigma) \cong
GW^{[n-1]}(X,-\can) \cong GW^{[n+1]}(X)$, recovering one of the results of
  \cite{Xie2018ATM}. In particular, it follows that
\[
\iHom(\Sigma^\infty (\mbb P^\sigma,[1:1]),\widetilde{GW}) \cong
GW^{[n+1]}
\]
as presheaves of spectra on $\Sm{S}^{C_2}$.
