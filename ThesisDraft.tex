...
\todo{Is Sing of a group complete $H$-space a group complete $H$-space?}
...
\xymatrix{\pi_0GW(R[t]) \ar@<0.5ex>[r] \ar@<-0.5ex>[r] & \pi_0GW(R)
  \ar[r] & \pi_0GW(\Delta R)}
\]
in the category of \emph{abelian} groups. 
...
\chapter{An $E_\infty$ structure on the Hermitian $K$-Theory Spectrum}\label{chap:Einf}
...
\item $KGL_{/X} \in SH_{/X}$, given a smooth map $T \rightarrow X$, get
  $Hom_{SH_{/X}}([T], KGL_{/X}) \cong KH(T)$
\item $f : X \rightarrow Y$, then $f^* KGL_{/Y} \cong KGL_{/X}$.
\end{theorem}

\begin{proof}
Given $E \in SH_{/X}$ and an abstract blowup square
\[
\xymatrix{\widetilde Z \ar[r]\ar[d] & \widetilde X \ar[d] \\ Z \ar[r] &X}
\]

with composite $q : \widetilde Z \rightarrow X$, get a Cartesian square
\[
\xymatrix{q_*q^* E & \ar[l] f_*f^* E \\ i_*i^*E \ar[u] & E \ar[l]\ar[u]}
\]

Apply to $E = KGL_{/X}$, now compute $Hom_{SH_{/X}}([X],-)$. To prove
that the square is Cartesian, pull-back and use the Cousin
sequence/base change. 
\end{proof}
...
We want to show that $\KR$ is a cdh sheaf on $\Sm{S}^{C_2}$. By Corollary 6.25 of \cite{HoyoisSixOp} it suffices to show that, for
